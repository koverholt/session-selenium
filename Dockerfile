...
RUN curl -O https://repo.anaconda.com/miniconda/Miniconda3-${MINICONDA_VERSION}-Linux-x86_64.sh && \
  bash Miniconda3-${MINICONDA_VERSION}-Linux-x86_64.sh -bp /opt/python/3.6.7 && \
  /opt/python/3.6.7/bin/pip install virtualenv && \
  rm -rf Miniconda3-${MINICONDA_VERSION}-Linux-x86_64.sh

ENV PATH="/opt/python/3.6.7/bin:${PATH}"

# Install Python packages -----------------------------------------------------#

# RUN /opt/python/3.6.7/bin/pip install \
#   beautifulsoup4 \
#   dash \
#   dask \
#   flask \
...
RUN /opt/python/3.6.7/bin/jupyter-nbextension install --sys-prefix --py rsp_jupyter && \
  /opt/python/3.6.7/bin/jupyter-nbextension enable --sys-prefix --py rsp_jupyter && \
  /opt/python/3.6.7/bin/jupyter-nbextension install --sys-prefix --py rsconnect_jupyter && \
  /opt/python/3.6.7/bin/jupyter-nbextension enable --sys-prefix --py rsconnect_jupyter && \
  /opt/python/3.6.7/bin/jupyter-serverextension enable --sys-prefix --py rsconnect_jupyter
